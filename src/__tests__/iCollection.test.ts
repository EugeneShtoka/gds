...
describe.each([() => new LinkedList<number>()])('Test collection interface for each implementing DS', (constructor) => {
    ...
    test(`${className}: remove method`, () => {
        ...
        expect(collection.remove(5).valueOf()).toBeFalsy();
        verifyNumberContentAndCount(collection, []);

        collection.add(4);
        verifyNumberContentAndCount(collection, [4]);

        expect(collection.remove(5)).toBeFalsy();
        verifyNumberContentAndCount(collection, [4]);

        expect(collection.remove(4)).toBeTruthy();
        verifyNumberContentAndCount(collection, []);
    ...
    test(`${className}: removeByPredicate method`, () => {
        ...
        for (let i = 0; i < 2; i++) {
            const result = collection.removeByPredicate(predicate(4));
